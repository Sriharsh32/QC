...
for i, (page_num, rect, text) in enumerate(all_blocks):
    ...
    for j, (p2, r2, t2) in enumerate(all_blocks):
        ...
        if abs(r2.y0 - rect.y1) < 15 and r2.intersects(rect + (0, 0, 200, 20)):
            ...
            # Fit notation like h7(-0.04 0) or P9(-0.02 -0.06)
    ...
    if tolerance_type == "Symmetric (±)":
        ...
        lower_bound = upper_bound = nominal
    ...
    data.append({
        ...
        "Lower Tol": float(low_tol),
        "Upper Tol": float(up_tol),
...
# Annotate PDF with dimension serial numbers
for item in data:
    ...
    below_y = rect.y1 + rect.height * 0.5 + font_size
...
# Save Excel (sorted by serial)
df = pd.DataFrame(data)
df = df.sort_values("Serial Number")
df = df[[
    "Serial Number", "Value", "Symbol", "Tolerance Type",
    "Lower Tol", "Upper Tol", "Lower Bound", "Upper Bound"
]]
